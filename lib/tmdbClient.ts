...
export const getGlobalAiringShows = async () => {
  try {
    ...
    futureDate.setDate(today.getDate() + 90); // 90 روز آینده

    const todayStr = today.toISOString().split('T')[0];
    const futureDateStr = futureDate.toISOString().split('T')[0];

    // دریافت همزمان ۳ صفحه اول (حدود ۶۰ سریال برتر جهان که تو این بازه پخش میشن)
    const p1 = fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&air_date.gte=${todayStr}&air_date.lte=${futureDateStr}&page=1`).then(r => r.json());
    const p2 = fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&air_date.gte=${todayStr}&air_date.lte=${futureDateStr}&page=2`).then(r => r.json());
    const p3 = fetch(`${BASE_URL}/discover/tv?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&air_date.gte=${todayStr}&air_date.lte=${futureDateStr}&page=3`).then(r => r.json());
    
    const [data1, data2, data3] = await Promise.all([p1, p2, p3]);
    
    const combinedData = [...(data1.results || []), ...(data2.results || []), ...(data3.results || [])];
    
    // حذف تکراری‌های احتمالی
    const uniqueShowsMap = new Map();
    combinedData.forEach((s: any) => { if (!uniqueShowsMap.has(s.id)) { uniqueShowsMap.set(s.id, s); } });
    const allUniqueIds = Array.from(uniqueShowsMap.keys());
    
    // دریافت جزئیات دقیق برای ۶۰ سریال
