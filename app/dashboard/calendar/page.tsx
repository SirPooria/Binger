...
export default function TrackerMainPage() {
  ...
  // حالت فعال: ادامه تماشا (Watch Next) یا تقویم پخش
  ...
  // تعداد روزهای گذشته که در بالای لیست لود می‌شوند (شروع از ۷ روز قبل به جای ۳۰ سال قبل!)
  ...
  // انیمیشن خروج کارت هنگام ثبت تماشا در ادامه تماشا
  ...
  const watchNextList = useMemo(() => {
    ...
    // سریال‌های آماده تماشا اول، و موارد منتظر پخش در انتها
    list.sort((a, b) => {
  ...
  // ۳. تقویم ارگونومیک (شروع مستقیم از دیروز و امروز، بدون پرش اسکرول)
  const calendarGroups = useMemo(() => {
    ...
    trackedShows.forEach(show => {
      ...
      validSeasons.forEach((season: any) => {
        ...
        seasonEps.forEach((ep: any) => {
          ...
          // در گذشته فقط تا محدوده تعیین شده (مثلاً ۷ یا ۳۰ روز قبل) را بالا می‌آوریم تا صفحه پرتاب نشود!
    ...
    // مرتب‌سازی زمانی از گذشته به آینده
    ...
    allEpisodes.forEach(item => {
      ...
      if (diffDays < -1) {
        // تاریخ‌های گذشته (مثلاً ۱۴ سپتامبر)
        ...
        // بعد از این هفته تا ۲۹۱ روز آینده
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#050505] text-white font-['Vazirmatn'] pb-28">
      ...
      {/* هدر کاملاً فیکس و قفل در بالا (هرگز گم نمی‌شود) */}
      ...
      <main className="max-w-4xl mx-auto px-4 md:px-8 mt-5">
        ...
        {/* ۱. حالت تقویم پخش (شروع طبیعی از دیروز و امروز، بدون پرش یا اسکرول مصنوعی) */}
        {isCalendarMode ? (
          <div className="space-y-6">

            {/* دکمه بارگذاری تاریخچه‌های قدیمی‌تر در بالای صفحه در صورت تمایل کاربر */}
                        ...
                        return (
                          <div
                            ...
                            className={`group flex items-center justify-between p-2.5 md:p-3 rounded-2xl border transition-all cursor-pointer ${
                              isWatched
                                ? 'bg-[#0a0a0a] border-white/5 opacity-50 hover:opacity-80' // تیره و مات برای دیده‌شده‌ها
                                : isAired
                                ? 'bg-[#181818] border-white/10 hover:border-[#ccff00]/50 shadow-md' // روشن و برجسته برای پخش‌شده‌های ندیده
                                : 'bg-[#121212] border-white/5 hover:border-white/15' // آینده (بدون دکمه تیک)
                            }`}
                          >
                            ...
                            <div className="flex items-center gap-3 shrink-0">
                              ...
                              {/* دکمه تیک فقط برای قسمت‌هایی که پخش شده‌اند فعال است */}
                              {isAired && (
                                <button
                                  ...
                                  title={isWatched ? "دیده‌شده (کلیک برای حذف)" : "ثبت دیدن"}
                                >
          ...
          /* ۲. حالت ادامه تماشا (Watch Next خالص و تمیز) */
