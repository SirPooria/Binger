...
export default function LoginPage() {
  ...
  // --- مرحله ۲: تایید کد و مسیریابی هوشمند ---
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // بررسی وضعیت آنبوردینگ کاربر از متادیتا
      ...
      setMessage('خطا: کد وارد شده اشتباه است.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-sm mx-auto">
        ...
        <div className="bg-black/20 border border-white/10 rounded-2xl p-8 backdrop-blur-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 mt-2 text-sm">با شماره تماس یا حساب گوگل خود وارد شوید.</p>
          </div>
          ...
          >
            {loading && step === 1 ? <Loader2 size={24} className="animate-spin" /> : <GoogleIcon />}
            ورود با گوگل
          </button>
          ...
          {step === 1 && (
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div>
                <label className="text-xs text-gray-500 mr-2 mb-1 block">شماره تماس</label>
                <input 
                  type="tel" 
                  required
                  value={phone}
          ...
          {step === 2 && (
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div>
                <label className="text-xs text-gray-500 mr-2 mb-1 block">کد تایید ارسال شده</label>
                <input 
                  type="text" 
                  required
                  value={otp}
                  onChange={(e) => setOtp(e.target.value)}
                  className="w-full bg-black/50 border border-white/10 rounded-xl p-3 text-white focus:border-[#ccff00] focus:outline-none transition-colors text-left ltr tracking-widest"
                  placeholder="----"
                  disabled={loading}
                />
              </div>
