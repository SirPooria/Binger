...
export default function Onboarding() {
  ...
  // قفل برای جلوگیری از ارسال ریکوئست‌های تکراری و کند شدن صفحه
  ...
  const lastShowElementRef = useCallback((node: HTMLDivElement) => {
    ...
    // اضافه کردن rootMargin برای لود شدن نرم‌تر قبل از رسیدن به انتهای اسکرول
  ...
  useEffect(() => {
    const loadShows = async () => {
      // اگر در حال دریافت دیتا هستیم، ریکوئست جدید نزن
      ...
      try {
        ...
        // فیلتر مهم: حذف سریال‌هایی که پوستر ندارند برای جلوگیری از ارور
        const validShows = newShows.filter((show: any) => show.poster_path);

        setShows(prevShows => {
          const combined = [...prevShows, ...validShows];
        ...
        isFetching.current = false; // باز کردن قفل
      }
    };
